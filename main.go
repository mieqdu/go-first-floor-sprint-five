...
func (r Running) Calories() float64 {
	speedCalories := CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift
	weight := r.Weight / MInKm
	duration := r.Duration.Hours() * MinInHours

	return speedCalories * weight * duration
}
...
func (w Walking) Calories() float64 {
	speedSquared := math.Pow(w.meanSpeed(), 2)
	weightCalories := CaloriesWeightMultiplier * w.Weight
	speedHeightCalories := (speedSquared / w.Height) * CaloriesSpeedHeightMultiplier * w.Weight
	duration := w.Duration.Hours() * MinInHours

	return (weightCalories + speedHeightCalories) * duration
}
...
func (s Swimming) meanSpeed() float64 {
	speed := float64(s.LengthPool * s.CountPool)

	return speed / MInKm / s.Duration.Hours()
}
...
func (s Swimming) Calories() float64 {
	speedCalories := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
	weightCalories := SwimmingCaloriesWeightMultiplier * s.Weight
	duration := s.Duration.Hours()

	return speedCalories * weightCalories * duration
}
